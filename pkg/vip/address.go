...
import (
	"fmt"
	"github.com/GCFactory/Patroni-BGP/pkg/iptables"
...
const (
	defaultValidLft = 60
	iptablesComment = "%s patroni-bgp load balancer IP"
)
...
func (configurator *network) insertIPTablesRulesForServicePorts(ipt *iptables.IPTables, vip, comment string) error {
	...
	for _, rule := range rules {
		// only handle rules with patroni-bgp comment
	...
	// iptables -A INPUT -d <vip> -p <protocol> --dport <port> -j ACCEPT -m comment —comment “<namespace/service-name> patroni-bgp load balancer IP”
